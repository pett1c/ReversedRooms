...
namespace Reversedrooms.UI
{
    public class CombatDisplay
    {
        ...
        public void Display(Character player, Location location)
        {
            ...
            Console.WriteLine($"[{location.Enemy.Name}]");
            Console.WriteLine($"[HP]: [{new string('█', location.Enemy.Health)}]");
            Console.WriteLine($"# {location.Enemy.Symbol}");
            Console.WriteLine($"# {location.Enemy.Description}");
        }
